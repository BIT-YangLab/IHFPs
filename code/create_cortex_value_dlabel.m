...
% DESCRIPTION:
%   Visualization of atlas with continuous values
%
% USAGE: 
%   cmp = zeros(17, 3);
%   create_cortex_label_dlabel(label, outfile, cmp, 0, 1)
%
...
%           cmp: color map
%
%           min_scale, max_scale: range of visualized value
% Outputs:  
%
% Written by Jinlong Li, Guoyuan Yang
    ...
    for row_i = 1:length(label)
        new_label(row_i, :) = ceil((label(row_i)-min_scale)/(max_scale-min_scale) * size(cmp, 1)) ;
    end
