function new_label = create_ref_groupROI(output_dir, group_label, ind_label_list)
...
g_list_cnt = length(ind_label_list);

for g_i = 1:g_list_cnt
    disp(['process ' num2str(g_i) '/' num2str(g_list_cnt)])
    
    % find homogeneity area 

    homo_pro_prior = find_homologous_area(ind_label_list{g_i});

    new_label = zeros(64984,1);
    MuI_threshhold_all_networks = findoverlapthreshold(homo_pro_prior, 1:400, 0,  'smooth_then_derivative');
    for i = 1:400 % parcel number: hard code (modify)
        if i == 54
            kk = 1;
        end
    ...
    if g_i == 1
        ...
        % new_label_l -> value 0 or label, do addition
        new_label_l = max(new_label_l, new_label);
    end
